...
class CourierLedgerLoader:
    ...
    def load_courier_ledger(self):
        ...
        with self.pg_dwh.connection() as conn:
            ...
            if not wf_setting:
                wf_setting = EtlSetting(id=0, workflow_key=self.WF_KEY, workflow_settings={self.LAST_LOADED_MONTH: {"year": 2023, "month": 5}})

            # Вычитываем очередную пачку объектов.
            last_loaded = wf_setting.workflow_settings[self.LAST_LOADED_MONTH]
            load_queue = self.stg.list_courier_ledgers(last_loaded, self.BATCH_LIMIT)
